...
int64_t _abot_itd_fp(Board *board, int64_t depth, const EvaluationTable *eval_table, const PredictionTable *predict_table, int64_t alpha, int64_t beta, TranspTable &transp_table, int64_t *iters);

int64_t negamax::abot_itd_fp(Board b, int64_t depth, const EvaluationTable *eval_table, const PredictionTable *predict_table, int64_t *iters)
{
    TranspTable transp_table;

    for (int64_t id = 1; id < depth; id++)
    {
        _abot_itd_fp(&b, id, eval_table, predict_table, LOSS, WIN, transp_table, iters);
    }

    return _abot_itd_fp(&b, depth, eval_table, predict_table, LOSS, WIN, transp_table, iters);
}
...
int64_t _abot_itd_fp(Board *board, int64_t depth, const EvaluationTable *eval_table, const PredictionTable *predict_table, int64_t alpha, int64_t beta, TranspTable &transp_table, int64_t *iters)
{
    ...
    for (Move &m : cached->moves)
    {
        ...
        int64_t score = _abot_itd_fp(board, depth - 1, eval_table, predict_table, new_alpha, -(best + SIGN(best)), transp_table, iters);
