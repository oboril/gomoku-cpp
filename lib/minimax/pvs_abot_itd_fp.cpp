...
int64_t negamax::pvs_abot_itd_fp(Board b, int64_t depth, const EvaluationTable *eval_table, const EvaluationTable *predict_table, int64_t *iters)
{
    ...
    {
        _pvs_abot_itd_fp(&b, id, eval_table, predict_table, LOSS, WIN, transp_table, iters);
    }
...
int64_t _pvs_abot_itd_fp(Board *board, int64_t depth, const EvaluationTable *eval_table, const EvaluationTable *predict_table, int64_t alpha, int64_t beta, TranspTable &transp_table, int64_t *iters)
{
    ...
    for (Move &m : cached->moves)
    {
        ...
        if (visited_children == 1)
        {
            ...
            int64_t score = _pvs_abot_itd_fp(board, depth - 1, eval_table, predict_table, new_alpha, new_beta, transp_table, iters);
        ...
        else
        {
            ...
            int64_t score = _pvs_abot_itd_fp(board, depth - 1, eval_table, predict_table, new_beta-1, new_beta, transp_table, iters);
            score = -score + SIGN(score);
            // if null window failed, redo the search
            if (score > best)
            {
                score = _pvs_abot_itd_fp(board, depth - 1, eval_table, predict_table, new_alpha, new_beta, transp_table, iters);
                score = -score + SIGN(score);
            }
            m.score = score;
            best = MAX(best, score);
