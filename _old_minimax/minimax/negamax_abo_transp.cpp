...
int64_t _negamax_abo_transp(Board *board, int64_t depth, const EvaluationTable * eval_table, const EvaluationTable * predict_table, int64_t alpha, int64_t beta, TranspTable &transp_table, int64_t *iters);
...
int64_t negamax_abo_transp(Board b, int64_t depth, const EvaluationTable * eval_table, const EvaluationTable * predict_table, int64_t *iters)
{
    ...
    return _negamax_abo_transp(&b, depth, eval_table, predict_table, LOSS, WIN, transp_table, iters);
}
...
int64_t _negamax_abo_transp(Board *board, int64_t depth, const EvaluationTable * eval_table, const EvaluationTable * predict_table, int64_t alpha, int64_t beta, TranspTable &transp_table, int64_t *iters)
{
    ...
    for (const Move m : cached->moves)
    {
        ...
        int64_t score = _negamax_abo_transp(board, depth - 1, eval_table, predict_table, new_alpha, -(best + SIGN(best)), transp_table, iters);
